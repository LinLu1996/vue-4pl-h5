      ...
      <van-pull-refresh v-model="isLoading" :disabled="refreshFlag" @refresh="onRefresh">
        <ul>
          ...
          >
            ...
            <focusContent :list="list" :loadingShow="loadingShow"></focusContent>
          </van-list>
        </ul>
      </van-pull-refresh>
    </div>
...
export default {
  ...
  methods: {
    ...
    getMainNav () {
      this.$http({
        url: this.$http.adornUrl('/userAttention/getParent.action'),
        method: 'get',
        params: this.$http.adornParams({
        }, false)
      }).then(({data}) => {
        this.mainNav = data.parent
        this.getSecondNav(0)
      })
    },
    // 点击一级菜单
    ...
    getSecondNav (index) {
      ...
      this.$http({
        url: this.$http.adornUrl('/userAttention/getAttentionProCounts.action'),
        method: 'get',
        params: this.$http.adornParams({
          // page: this.pageIndex,
    ...
    getList (index) {
      ...
      this.$http({
        ...
        url: this.$http.adornUrl('/product/getAttention.action'),
        method: 'get',
        params: this.$http.adornParams({
          page: this.pageIndex,
          pageSize: this.pageSize,
